...
class Game {
  constructor() {
    ...
    this.terrainTheme = 'grass';
    this.terrainSeed  = Math.floor(Math.random() * 100000);
    this.terrainCache = Object.create(null);
  ...
  _drawBg() {
    const ctx = this.ctx;
    const theme = this._getTerrainTheme();
    ...
    ctx.save();
    const pattern = this._getTerrainPattern(theme.key);
    if (pattern) {
      ctx.globalAlpha = 0.7;
      ctx.fillStyle = pattern;
      ctx.fillRect(GRID_OX, GRID_OY, GRID_COLS * CELL_W, GRID_ROWS * CELL_H);
    }

    const shade = ctx.createLinearGradient(0, GRID_OY, 0, GRID_OY + GRID_ROWS * CELL_H);
    shade.addColorStop(0, theme.topShade);
    shade.addColorStop(0.5, 'rgba(0,0,0,0.04)');
    shade.addColorStop(1, theme.bottomShade);
    ctx.fillStyle = shade;
    ctx.fillRect(GRID_OX, GRID_OY, GRID_COLS * CELL_W, GRID_ROWS * CELL_H);
    ctx.restore();
  }
  ...
  _drawUnitShape(ctx, typeKey, color, size, scale, hit) {
    ...
    const base = hit ? '#ffffff' : color;
    ctx.fillStyle = base;
    ctx.strokeStyle = 'rgba(255,255,255,0.95)';
    ctx.lineWidth = 1;

    if (typeKey === 'grunt') {
      this._drawInfantry(ctx, s, base);
      return;
    }
    if (typeKey === 'ranger') {
      this._drawRanger(ctx, s, base);
      return;
    }
    if (typeKey === 'speeder') {
      this._drawCrawler(ctx, s, base);
      return;
    }
    if (typeKey === 'sniper') {
      this._drawSniper(ctx, s, base);
      return;
    }
    if (typeKey === 'tank') {
      this._drawTank(ctx, s, base);
      return;
    }
    if (typeKey === 'artillery') {
      this._drawArtillery(ctx, s, base);
      return;
    }
  ...
  _drawInfantry(ctx, s, base) {
    ctx.save();
    ctx.fillStyle = this._mixColor(base, 0.12);
    ctx.beginPath();
    ctx.arc(0, -s * 0.95, s * 0.34, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();

    this._roundRect(ctx, -s * 0.52, -s * 0.58, s * 1.04, s * 1.25, s * 0.18);
    ctx.fill();
    ctx.stroke();
  ...
  _drawRanger(ctx, s, base) {
    ctx.save();
    this._drawInfantry(ctx, s * 0.92, base);

    ctx.strokeStyle = '#e9f9ff';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(-s * 0.98, -s * 0.1);
    ctx.lineTo(s * 0.18, s * 0.32);
    ctx.stroke();

    ctx.lineWidth = 1.2;
    ctx.beginPath();
    ctx.moveTo(-s * 0.76, -s * 0.55);
  ...
  _drawCrawler(ctx, s, base) {
    ctx.save();
    ctx.fillStyle = this._mixColor(base, 0.05);
    this._roundRect(ctx, -s * 1.15, -s * 0.42, s * 2.3, s * 0.84, s * 0.28);
    ctx.fill();
    ctx.stroke();

    ctx.beginPath();
    ctx.moveTo(-s * 1.24, -s * 0.1);
    ctx.lineTo(-s * 1.68, -s * 0.3);
    ctx.moveTo(-s * 1.24, s * 0.1);
    ctx.lineTo(-s * 1.68, s * 0.3);
    ctx.moveTo(s * 1.24, -s * 0.1);
    ctx.lineTo(s * 1.68, -s * 0.3);
    ...
    ctx.beginPath();
    ctx.arc(s * 0.25, -s * 0.05, s * 0.16, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }

  _drawSniper(ctx, s, base) {
    ctx.save();
    this._drawInfantry(ctx, s * 0.9, base);
    ctx.lineWidth = 2;
    ...
    ctx.fillRect(s * 0.3, -s * 0.22, s * 0.24, s * 0.14);
    ctx.restore();
  }

  _drawTank(ctx, s, base) {
    ctx.save();
    ctx.fillStyle = this._mixColor(base, 0.08);
    this._roundRect(ctx, -s * 1.1, -s * 0.52, s * 2.2, s * 1.04, s * 0.2);
    ctx.fill();
    ctx.stroke();

    ctx.fillStyle = this._mixColor(base, -0.12);
    this._roundRect(ctx, -s * 0.62, -s * 0.28, s * 1.24, s * 0.56, s * 0.15);
    ctx.fill();
    ctx.stroke();
    ...
    ctx.lineTo(s * 1.55, -s * 0.06);
    ctx.stroke();

    ctx.fillStyle = this._mixColor(base, -0.28);
    this._roundRect(ctx, -s * 1.2, s * 0.46, s * 2.4, s * 0.24, s * 0.08);
    ctx.fill();
    this._roundRect(ctx, -s * 1.2, -s * 0.7, s * 2.4, s * 0.24, s * 0.08);
    ctx.fill();
    ctx.restore();
  }

  _drawArtillery(ctx, s, base) {
    ctx.save();
    ctx.fillStyle = this._mixColor(base, 0.02);
    this._roundRect(ctx, -s * 1.25, -s * 0.58, s * 2.5, s * 1.16, s * 0.2);
    ctx.fill();
    ctx.stroke();

    ctx.fillStyle = this._mixColor(base, -0.16);
    this._roundRect(ctx, -s * 0.48, -s * 0.36, s * 0.96, s * 0.72, s * 0.12);
    ctx.fill();
    ctx.stroke();
    ...
    ctx.lineTo(s * 1.9, -s * 0.22);
    ctx.stroke();

    ctx.fillStyle = '#f4fbff';
    ctx.fillRect(s * 1.84, -s * 0.28, s * 0.2, s * 0.1);
    ctx.restore();
  }

  _mixColor(hex, amount) {
  ...
  _pickTerrainTheme() {
    const themes = ['grass', 'desert'];
    this.terrainTheme = themes[Math.floor(Math.random() * themes.length)];
    this.terrainSeed = Math.floor(Math.random() * 100000);
  }

  _getTerrainTheme() {
    if (this.terrainTheme === 'desert') {
      return {
        key: 'desert',
        skyTop: '#2f1e11',
        skyBottom: '#120d09',
        topShade: 'rgba(255, 182, 86, 0.10)',
        bottomShade: 'rgba(130, 70, 30, 0.16)',
        p1Zone: 'rgba(85, 165, 255, 0.15)',
        p2Zone: 'rgba(255, 95, 55, 0.16)',
      };
    }
    ...
    return {
      ...
      skyTop: '#132b1c',
      skyBottom: '#08120d',
      topShade: 'rgba(120, 194, 110, 0.10)',
      bottomShade: 'rgba(38, 82, 36, 0.16)',
      p1Zone: 'rgba(85, 160, 255, 0.16)',
      p2Zone: 'rgba(255, 80, 80, 0.14)',
    };
  }

  _getTerrainPattern(themeKey) {
    const cacheKey = `${themeKey}:${this.terrainSeed % 11}`;
    if (this.terrainCache[cacheKey]) return this.terrainCache[cacheKey];
    ...
    if (themeKey === 'desert') {
      const sand = tctx.createLinearGradient(0, 0, 0, tex.height);
      sand.addColorStop(0, '#8e6a42');
      sand.addColorStop(1, '#6f5334');
      tctx.fillStyle = sand;
      tctx.fillRect(0, 0, tex.width, tex.height);

      for (let i = 0; i < 32; i++) {
        tctx.strokeStyle = `rgba(250, 220, 155, ${0.04 + (i % 5) * 0.01})`;
        tctx.lineWidth = 1;
        tctx.beginPath();
        const y = (i * 7 + (this.terrainSeed % 13)) % tex.height;
        tctx.moveTo(0, y);
        tctx.bezierCurveTo(50, y - 6, 160, y + 6, tex.width, y);
        tctx.stroke();
      }

      for (let i = 0; i < 170; i++) {
        const x = (i * 37 + this.terrainSeed * 3) % tex.width;
        const y = (i * 61 + this.terrainSeed * 5) % tex.height;
        tctx.fillStyle = i % 3 === 0 ? 'rgba(80,56,32,0.25)' : 'rgba(255,226,171,0.2)';
        tctx.fillRect(x, y, 2, 2);
      }
    } else {
      const grass = tctx.createLinearGradient(0, 0, 0, tex.height);
      grass.addColorStop(0, '#315d2f');
      grass.addColorStop(1, '#244924');
      tctx.fillStyle = grass;
      tctx.fillRect(0, 0, tex.width, tex.height);

      for (let i = 0; i < 220; i++) {
        const x = (i * 29 + this.terrainSeed * 7) % tex.width;
        const y = (i * 17 + this.terrainSeed * 11) % tex.height;
        const bladeH = 4 + (i % 6);
        tctx.strokeStyle = i % 4 === 0 ? 'rgba(188,225,130,0.28)' : 'rgba(42,78,35,0.34)';
        tctx.lineWidth = 1;
        tctx.beginPath();
        tctx.moveTo(x, y + bladeH);
        tctx.lineTo(x + ((i % 2) ? 1 : -1), y);
        tctx.stroke();
      }

      for (let i = 0; i < 120; i++) {
        const x = (i * 43 + this.terrainSeed * 13) % tex.width;
        const y = (i * 23 + this.terrainSeed * 17) % tex.height;
        tctx.fillStyle = i % 5 === 0 ? 'rgba(225, 247, 170, 0.18)' : 'rgba(36, 66, 34, 0.18)';
        tctx.beginPath();
        tctx.arc(x, y, 1.3, 0, Math.PI * 2);
        tctx.fill();
      }
    }

    const pattern = this.ctx.createPattern(tex, 'repeat');
    this.terrainCache[cacheKey] = pattern;
    return pattern;
  }
